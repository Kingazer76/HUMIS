...
describe('GET/PUT /api/settings', () => {
  ...
  it('changing tank capacity changes tank percentage on GET /api/water', async () => {
    const before = await agent.get('/api/water')
    ...
    const after = await agent.get('/api/water')
  ...
  it('shrinking capacity below the current litres clamps stored water and lowers days remaining', async () => {
    const start = await agent.post('/api/irrigation/zone-a/start').send()
    expect(start.body.ok).toBe(true)
    simulatedProvider?.tick(60)

    const before = await agent.get('/api/planning')
    ...
    const water = await agent.get('/api/water')
    ...
    const after = await agent.get('/api/planning')
  ...
  it('rejects invalid tank numbers and leaves the live settings unchanged', async () => {
    const rejected = await agent.put('/api/settings/tank').send({
      capacityL: -10,
      lowThresholdPct: 25,
      criticalThresholdPct: 15,
    })
    ...
    const stillDefault = await agent.get('/api/water')
    expect(stillDefault.body.tank.capacityL).toBe(15000)
    expect(getTankConfig()).toEqual(TANK_CONFIG)

    const equalThresholds = await agent.put('/api/settings/tank').send({
      capacityL: 15000,
      lowThresholdPct: 20,
  ...
  it('keeps a later GET of settings and water on the saved tank numbers (runtime persistence)', async () => {
    await agent.put('/api/settings/tank').send({
      capacityL: 12000,
      lowThresholdPct: 40,
      criticalThresholdPct: 20,
    })
  ...
  it('renames a zone and changes its crop, then GET /api/zones reflects it', async () => {
    const saved = await agent.put('/api/settings/zones/zone-a').send({
  ...
  it('rejects moisture min at or above max without changing the zone', async () => {
    const rejected = await agent.put('/api/settings/zones/zone-a').send({
      name: 'Zone A — North Field',
      cropId: 'maize',
      irrigationPreference: 'standard',
      overrideMinPct: 70,
  ...
  it('saves a watering style on a zone', async () => {
    const saved = await agent.put('/api/settings/zones/zone-b').send({
  ...
  it('reset restores a zone to the seeded defaults', async () => {
    await agent.put('/api/settings/zones/zone-a').send({
      name: 'Temporary name',
      cropId: 'tomato',
    ...
    const reset = await agent.post('/api/settings/zones/zone-a/reset').send()
  ...
  it('restoring tank defaults returns the seeded 15,000 L / 25% / 15%', async () => {
    ...
    const restored = await agent.post('/api/settings/tank/defaults').send()
