...
/**
 * Used when Azure Speech is not configured. Never invents a transcript.
 */
export class UnavailableSpeechToTextProvider implements SpeechToTextProvider {
  async transcribe(_audio: Buffer, _contentType: string): Promise<SpeechToTextResponse> {
    return {
      ok: false,
      reason:
        'Speech listening is not set up yet. Add AZURE_SPEECH_KEY and AZURE_SPEECH_REGION on the server, then try again.',
    }
  }
}
